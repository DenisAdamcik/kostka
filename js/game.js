...
document.getElementById('addCube').addEventListener('click', function() {
    if (document.querySelectorAll('.cube').length < 3) {
        addCube();
    }
});
...
function addCube() {
    var newCubeId = 'cube' + (document.querySelectorAll('.cube').length + 1);
    var newCube = document.createElement('img');
    newCube.id = newCubeId;
    newCube.src = 'img/kostka6.png';
    newCube.alt = 'Kostka';
    newCube.classList.add('cube');
    document.getElementById('diceContainer').appendChild(newCube);
}
